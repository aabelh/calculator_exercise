...
public class PriceRepository {
    ...
    public Price priceByArticleId(String articleId) {
        return prices.computeIfAbsent(articleId,
                key -> new Price(BigDecimal.valueOf(0.5d + random.nextDouble() * 29.50d)
    ...
    public Price priceByArticleIdAndCustomerId(String articleId, String customerId) {
        switch(customerId) {
            case "customer-1":
                return new Price(priceByArticleId(articleId).getValue()
            ...
            default:
                throw new PriceNotFoundException(
                        String.format("Price not found for articleId=%s and customerId=%s", articleId, customerId)
                );
        }
